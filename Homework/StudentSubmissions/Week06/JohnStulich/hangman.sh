...
trap 'echo "Ctrl-C ignored. Find another way to quit (hint: ct z)" ' INT
...
if [ ! -r "$wordlib" ] ; then
  ...
  echo "(online: https://raw.githubusercontent.com/brandonprry/wicked_cool_shell_scripts_2e/master/long-words.txt" >&2
...
while [ "$guess" != "quit" ] ; do
  match="$(./randomquote.sh $wordlib)"      # pick a new word from the library

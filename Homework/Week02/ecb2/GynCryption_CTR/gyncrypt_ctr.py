...
# modified to use CTR mode with initial counter=0
...
source_file, target_file, result_file = "source.pdf", "source.jpg", "gyncryption_ctr.pdf"
...
def brute(s, h, BS=16):
    ...
    while (i<END):
        ...
        c = AES.new(key, AES.MODE_CTR, counter=ctr).encrypt(p)
        if c.startswith(h):
            print "key=", key
...
ctr = Counter.new(128, initial_value=0)
result = AES.new(key, AES.MODE_CTR, counter=ctr).encrypt(s)
...
with open("decrypt-gyn_ctr.py", "wb") as f:
...
ctr = Counter.new(128, initial_value=0)
d = AES.new(%(key)s, AES.MODE_CTR, counter=ctr).encrypt(d)
